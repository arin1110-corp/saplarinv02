...
class Controller extends BaseController
{
    ...
    public function loginUser(Request $request)
    {
        ...
        $user = ModelUser::where('user_nip', $request->nip)->orWhere('user_email', $request->nip)->first();

        // Step 1: NIP/Email tidak ditemukan
        if (!$user) {
            return back()
                ->withErrors(['nip' => 'NIP atau Email tidak ditemukan'])
                ->withInput();
        }
    ...
    public function loginVerifikator(Request $request)
    {
        ...
        $verifikator = ModelVerificator::where('verificator_nip', $request->nip)->orWhere('verificator_email', $request->nip)->first();
