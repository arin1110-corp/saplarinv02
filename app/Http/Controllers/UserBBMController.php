...
class UserBBMController extends Controller
{
    ...
    {
        $bbms = ModelBBM::with('subKegiatan')->where('bbm_pengaju_id', session('pegawai_id'))->orderBy('created_at', 'desc')->get();
    ...
    public function create()
    {
        ...
        return view('user.bbm.create', compact('subKegiatans'));
    }

    public function store(Request $request, BBMEmailService $emailService)
    {
        $request->validate([
            'bbm_sub_kegiatan_id' => 'required',
            'bbm_uraian_kegiatan' => 'required',

            'bbm_liter' => 'required|numeric|min:0.01',

            'bbm_spt_file' => 'required|mimes:pdf,jpg,jpeg,png|max:5120',
        ]);

        $uid = (string) Str::uuid();

        $sptFile = $this->uploadFileToPublic($request->file('bbm_spt_file'), $uid, 'spt');
        ...
        ModelBBM::create([
            ...
            'bbm_bidang_nama' => session('pegawai_bidang_nama'),

            'bbm_sub_kegiatan_id' => $request->bbm_sub_kegiatan_id,
        ...
        $bbm->load('subKegiatan');

        $emailService->kirimKeAdminBBM('Pengajuan BBM Baru - ' . $bbm->bbm_pengaju_nama, "Yth. Admin BBM,\n\n" . "Terdapat pengajuan BBM baru dengan data berikut:\n\n" . "Nama Pengaju : {$bbm->bbm_pengaju_nama}\n" . "NIP          : {$bbm->bbm_pengaju_nip}\n" . "Bidang       : {$bbm->bbm_bidang_nama}\n" . 'Sub Kegiatan : ' . ($bbm->subKegiatan->sub_kegiatan_nama ?? '-') . "\n" . "Status       : {$bbm->bbm_status_pengajuan}\n\n" . "Silakan login ke SAPLARIN untuk melakukan verifikasi.\n\n" . 'SAPLARIN');

        $emailService->kirimKePengaju($bbm, 'Pengajuan BBM Berhasil Dikirim', "Yth. {$bbm->bbm_pengaju_nama},\n\n" . "Pengajuan BBM Anda berhasil dikirim.\n\n" . "Status       : {$bbm->bbm_status_pengajuan}\n" . 'Sub Kegiatan : ' . ($bbm->subKegiatan->sub_kegiatan_nama ?? '-') . "\n\n" . "Mohon menunggu proses verifikasi admin.\n\n" . 'SAPLARIN');

        return redirect()->route('user.bbm.index')->with('success', 'Pengajuan BBM berhasil dikirim');
    }

    public function show($uid)
    {
        $bbm = ModelBBM::with('subKegiatan')->where('bbm_uid', $uid)->where('bbm_pengaju_id', session('pegawai_id'))->firstOrFail();
    ...
    public function uploadLaporan(Request $request, $uid, BBMEmailService $emailService)
    {
        ...
        $notaFile = $this->uploadFileToPublic($request->file('bbm_laporan_nota_file'), $bbm->bbm_uid, 'nota', $request->bbm_tanggal_nota);
    ...
    private function uploadFileToPublic($file, $uid, $jenis, $tanggalNota = null)
    {
        ...
        $tanggal = $tanggalNota ? date('Ymd', strtotime($tanggalNota)) : date('Ymd');
