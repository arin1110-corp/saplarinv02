...
class BBMApiController extends Controller
{
    /**
     * Daftar pengajuan BBM milik user login.
     */
    public function index(Request $request): JsonResponse
    {
        $user = $request->user();

        $bbms = ModelBBM::where('bbm_pengaju_id', $user->user_uid)->orderBy('created_at', 'desc')->get();
    ...
    public function show(Request $request, string $uid): JsonResponse
    {
        ...
        $bbm = ModelBBM::where('bbm_uid', $uid)->where('bbm_pengaju_id', $user->user_uid)->first();

        if (!$bbm) {
            return response()->json(
                [
                    'status' => false,
                    'message' => 'Data pengajuan BBM tidak ditemukan.',
                ],
                404,
            );
        }
    ...
    public function store(Request $request, BBMEmailService $emailService, ArinDriveService $arinDrive): JsonResponse
    {
        ...
        */
        ...
        $sptFile = $arinDrive->upload($request->file('bbm_spt_file'), 'bbm_spt', $uid . '_SPT.' . $request->file('bbm_spt_file')->getClientOriginalExtension(), $uid);
        ...
        */
        ...
        $fotoMobilFile = $arinDrive->upload($request->file('bbm_foto_mobil_file'), 'bbm_foto_mobil', $uid . '_FOTO_MOBIL.' . $request->file('bbm_foto_mobil_file')->getClientOriginalExtension(), $uid);
        ...
        */
        ...
        $buktiTambahan = $this->uploadBuktiTambahan($request, $arinDrive, $uid, 'PENGAJUAN');
        ...
        */
        ...
        $bbm = ModelBBM::create([
            ...
            'bbm_pengaju_id' => $user->user_uid,
            'bbm_pengaju_nama' => $user->user_nama ?? null,
            'bbm_pengaju_nip' => $user->user_nip ?? null,
            'bbm_pengaju_email' => $user->user_email ?? null,

            'bbm_bidang_nama' => $user->user_bidang ?? ($user->user_bidang_nama ?? '-'),
        ...
        */
        ...
        return response()->json(
            [
                'status' => true,
                'message' => 'Pengajuan BBM berhasil dikirim.',
                'data' => $bbm,
            ],
            201,
        );
     ...
     * Upload laporan nota.
     */
    public function uploadLaporan(Request $request, string $uid, BBMEmailService $emailService, ArinDriveService $arinDrive): JsonResponse
    {
        $request->validate([
        ...
        if ($bbm->bbm_status_pengajuan !== 'Pengajuan Diterima') {
            return response()->json(
                [
                    'status' => false,
                    'message' => 'Upload nota hanya bisa dilakukan setelah pengajuan diterima.',
                ],
                422,
            );
        }
        ...
        if ($bbm->bbm_status_laporan === 'Laporan Nota Diterima') {
            return response()->json(
                [
                    'status' => false,
                    'message' => 'Laporan nota sudah diterima dan tidak bisa diubah.',
                ],
                422,
            );
        }
        ...
        $notaFile = $arinDrive->upload($request->file('bbm_laporan_nota_file'), 'bbm_nota', $bbm->bbm_uid . '_NOTA_' . $tanggal . '.' . $request->file('bbm_laporan_nota_file')->getClientOriginalExtension(), $bbm->bbm_uid);
        ...
        $buktiBaru = $this->uploadBuktiTambahan($request, $arinDrive, $bbm->bbm_uid, 'NOTA');
        ...
        return response()->json([
            'status' => true,
            'message' => 'Laporan nota berhasil diupload.',
            'data' => $bbm->fresh(),
        ]);
    }
    ...
    private function uploadBuktiTambahan(Request $request, ArinDriveService $arinDrive, string $uid, string $jenis): array
    {
        ...
        foreach ($request->file('bbm_bukti_tambahan') as $index => $file) {
            ...
            $url = $arinDrive->upload($file, 'bbm_bukti', $uid . '_BUKTI_' . $jenis . '_' . ($index + 1) . '_' . time() . '.' . $extension, $uid);
