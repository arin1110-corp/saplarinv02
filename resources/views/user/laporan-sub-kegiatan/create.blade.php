...
<div class="max-w-6xl space-y-6">
    ...
    <div class="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl shadow-lg p-6 text-white">
        <h2 class="text-2xl font-bold">
            Input Laporan Sub Kegiatan
        </h2>
        <p class="text-blue-100 text-sm mt-2">
            Pilih sub kegiatan, isi realisasi indikator, permasalahan, solusi, dan tindak lanjut.
        </p>
    </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="laporan_sub_kegiatan_id"
                        id="subKegiatanSelect"
                        onchange="loadIndikator()"
                        class="w-full rounded-2xl border border-slate-200 px-4 py-3"
                        required>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div class="mb-5">
                ...
                <p class="text-sm text-slate-500">
                    Indikator otomatis muncul berdasarkan master indikator yang dibuat admin.
                </p>
            </div>
...
<script>
    function loadIndikator() {
        ...
        fetch("{{ url('/user/laporan-sub-kegiatan/indikator') }}/" + subKegiatanId)
            .then(response => response.json())
            .then(data => {
                if (!data.length) {
                    container.innerHTML = `
                        <div class="bg-red-50 border border-red-200 rounded-2xl p-5 text-red-700">
                            Belum ada indikator aktif untuk sub kegiatan ini. Hubungi admin.
                        </div>
                    `;
                    return;
                }
